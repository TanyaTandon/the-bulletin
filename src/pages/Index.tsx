...
const Index = () => {
  ...
  const handleVerifySignIn = async (event: React.MouseEvent<HTMLButtonElement>) => {
    if (!code) return;
          ...
          <Tabs defaultValue="signin" className="w-full">
            ...
            <TabsContent value="signin">
              <form onSubmit={handleSignIn} className="space-y-4">
                      ...
                      <InputOTPGroup>
                        {slots.map((slot, index) => (
                          <InputOTPSlot 
                            key={index} 
                            {...slot} 
                            className={isMobile ? "h-10 w-10 md:h-12 md:w-12 text-lg" : "h-12 w-12 text-lg"}
                          />
                        ))}
                      </InputOTPGroup>
                    )}
                ...
                <Button onClick={handleVerifySignIn} className="w-full" disabled={isLoading}>
