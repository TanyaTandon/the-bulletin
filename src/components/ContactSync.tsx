...
const ContactSync: React.FC<ContactSyncProps> = ({ showResults = false }) => {
  ...
  const [syncedContacts, setSyncedContacts] = useState<Array<{
    name: string,
    phone: string,
    inDatabase: boolean
  }>>([]);
  ...
  const handleOpenContacts = async () => {
    ...
    setTimeout(() => {
      ...
      const mappedContacts = deviceContacts.map(contact => {
        const existingUser = existingUsers.find(u => u.phone === contact.phone);
        return {
          ...contact,
          selected: false,
        ...
        <div className="mt-6 rounded-md border p-4">
          ...
          <div className="space-y-2 max-h-[300px] overflow-y-auto">
            {syncedContacts.map((contact, idx) => (
              <div key={idx} className="flex items-center justify-between p-2 rounded-md hover:bg-gray-50">
